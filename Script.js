...
// Manual-only relay control
if (String(topic) == "ats/cmd/mainRelay") {
  // Remove the manualMode check or change it to allow in AUTO too
  digitalWrite(RELAY_MAIN, cmd == "1" ? HIGH : LOW);
  Serial.println("Main relay set (manual override)");
  return;
}

if (String(topic) == "ats/cmd/genRelay") {
  digitalWrite(RELAY_GEN, cmd == "1" ? HIGH : LOW);
  Serial.println("Gen relay set (manual override)");
  return;
}

if (String(topic) == "ats/cmd/startGen") {
  digitalWrite(RELAY_START_GEN, cmd == "1" ? HIGH : LOW);
  Serial.println("Start generator set (manual override)");
  return;
}